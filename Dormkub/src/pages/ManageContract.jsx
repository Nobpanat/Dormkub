          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {contracts.map((contract) => (
                ...
                >
                  <h3 className="text-xl font-bold">{contract.DormitoryName}</h3>
                  ...
                  <p>วันที่เริ่มต้น: {new Date(contract.startDate).toLocaleDateString()}</p>
                  <p>วันที่สิ้นสุด: {new Date(contract.endDate).toLocaleDateString()}</p>
                </div>
              ))}
            </div>
          )}
